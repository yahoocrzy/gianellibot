...
class ChannelSelectView(discord.ui.View):
    ...
    async def _complete_setup(self, interaction: discord.Interaction, notification_channel_id: Optional[int]):
        ...
        embed = EmbedFactory.create_success_embed(
            "⚠️ Legacy Setup Complete",
            f"Your ClickUp integration is configured using the old system.{channel_text}\n\n"
            "**🆕 Important: We recommend migrating to the new system!**\n\n"
            "The new multi-workspace system offers:\n"
            "• Support for multiple ClickUp workspaces\n"
            "• Full dropdown selections (no more typing IDs!)\n"
            "• Enhanced calendar views\n"
            "• Better security\n\n"
            "**To migrate:**\n"
            "1. Use `/workspace-add` to add your workspace\n"
            "2. All new commands will work automatically\n\n"
            "**✅ Ready to use immediately after setup:**\n"
            "• `/calendar` - View ALL tasks in calendar format\n"
            "• `/upcoming` - See ALL upcoming tasks\n"
            "• `/today` - View ALL today's tasks\n"
            "• `/task-create` - Create tasks with dropdowns\n"
            "• `/task-update` - Update tasks interactively\n"
            "• `/task-list` - View tasks with filtering\n"
            "• `/task-delete` - Delete with confirmation\n\n"
            "**Setup AI Features:**\n"
            "• `/claude-setup` - Enable AI-powered features\n\n"
            "**Workspace Management:**\n"
            "• `/workspace-add` - Add ClickUp workspaces\n"
...
class SetupWizard(commands.Cog):
    ...
    async def clickup_setup(self, interaction: discord.Interaction):
        ...
        embed = EmbedFactory.create_info_embed(
            ...
            "Use `/workspace-add` to add your first workspace (supports multiple workspaces!)\n\n"
            "**Or use the legacy setup:**\n"
            "The old `/clickup-setup` wizard still works but only supports one workspace."
        )
